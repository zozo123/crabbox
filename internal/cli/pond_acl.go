...
func pondACLMergePolicy(body, tag string) (string, error) {
	...
	// trailing commas). The stdlib json package rejects those; without this
	// normalization step the first crewACLEnsure against a real tailnet fails
	// silently (the merge errors out, the PUT never happens, peers join the
	// tailnet but ACL traffic is blocked — a silent security gap). Try to
	// standardize first; fall through to plain JSON on success.
